...
- [Policy-Windows-NIST-800-53.md](Policy-Windows-NIST-800-53.md) maps the 134 settings of the Windows base profile and the No Store Apps layer to NIST SP 800-53 Revision 5, Release 5.2.0. It records which control objectives the baseline supports and which it leaves unaddressed, which settings do not take effect on which Windows versions and editions, the seven settings that improve one control objective at the cost of another, and the 28 catalog settings the baseline declines, each with the control objective the exclusion protects.
