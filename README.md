...
[Policy-Windows-Base.psd1](profiles/Policy-Windows-Base.psd1) ([reference doc](profiles/reference/Policy-Windows.md)) is a privacy and security baseline for standalone devices, with 112 settings covering telemetry, app permissions, activity history, content delivery, and default-behavior security hardening. [Policy-Windows-NoStoreApps.psd1](profiles/Policy-Windows-NoStoreApps.psd1) is an additive layer for devices that do not use the Microsoft Store or its apps, with 22 settings disabling the Microsoft account, OneDrive, Store, cross-device services, and core app permissions. The reference document records what the base includes and excludes, the device each layer assumes, and the verdicts behind its privacy-versus-security tradeoffs such as Defender cloud protection and Find My Device.

[Policy-Edge-Base.psd1](profiles/Policy-Edge-Base.psd1) ([reference doc](profiles/reference/Policy-Edge.md)) is a privacy and security baseline for Microsoft Edge, with 117 settings covering telemetry and tracking, Copilot and AI, sign-in and data retention, browser hardening, and Microsoft-feature removals. [Policy-Edge-NoWebConferencing.psd1](profiles/Policy-Edge-NoWebConferencing.psd1) is an additive layer for devices that do not use web conferencing, with 3 settings blocking microphone, camera, and screen capture. The reference document records what the Edge base includes and excludes, the device each layer assumes, and the verdicts behind its privacy-versus-security tradeoffs, led by SmartScreen.
